...
def save_reinitialization_data_to_csv(csv_filename, reinitialization_data):
    folder_path = os.path.join(current_dir, 'reinitialization_data')
...
#reinitialization_data = load_reinitialization_data(reinitialization_data_filename)
...
for i in range(len(label_container_abs)):
    ...
    for j in range(TRACKER_NUM):
        ...
        if success:
            ...
            name_sequence.append(tracker_type_name[j])
    ...
    if position_estimates:
        ...
        print(f'filtered position estimate: \n{filtered_position_estimates}')
        print(f'tracker name: {name_sequence}')
...
if not break_flag and reinitialization_data_new:
    # Predefined CSV file name.
    csv_filename = reinitialization_data_filename
    
    # Check if a filename is available.
    if not csv_filename:
        ...
        # Otherwise, use the predefined file name.
